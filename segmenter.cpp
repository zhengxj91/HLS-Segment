...
static AVStream *add_output_stream(AVFormatContext *output_format_context, AVStream *input_stream) {
	...
	if (av_q2d(input_codec_context->time_base) * input_codec_context->ticks_per_frame
			> av_q2d(input_stream->time_base) && av_q2d(input_stream->time_base) < 1.0 / 1000) {
...
int write_index_file(const char *index, const char *tmp_index, unsigned int planned_segment_duration,
		unsigned int numsegments, unsigned int *actual_segment_duration, unsigned int segment_number_offset,
...
int segment(char *input_file, char* base_dirpath, char* output_index_file, char *base_file_name,
		char* base_file_extension, int segmentLength, int listlen) {
	...
	snprintf(currentOutputFileName, MAX_FILENAME_LENGTH, "%s/%s-%u%s", base_dirpath, base_file_name, output_index,
			base_file_extension);
	...
	do {
		...
		if (iskeyframe && ((packet_time - segment_start_time) >= (segmentLength - 0.25))) { //a keyframe  near or past segmentLength -> SPLIT
			...
			actual_segment_durations[num_segments] = (unsigned int) rint(
					prev_packet_time - segment_start_time);
			num_segments++;
			if (num_segments > listlen) { //move list to exclude last:
				snprintf(currentOutputFileName, MAX_FILENAME_LENGTH, "%s/%s-%u%s", base_dirpath,
						base_file_name, listofs, base_file_extension);
			...
			write_index_file(output_index_file, tmp_output_index_file, segmentLength, num_segments,
					actual_segment_durations, listofs, base_file_name, base_file_extension,
					(num_segments >= MAX_SEGMENTS));

			if (num_segments == MAX_SEGMENTS) {
				fprintf(stderr,
						"Reached \"hard\" max segment number %u. If this is not live stream increase segment duration. If live segmenting set max list lenth (-m ...)\n",
						MAX_SEGMENTS);
				break;
			}
			output_index++;
			snprintf(currentOutputFileName, MAX_FILENAME_LENGTH, "%s/%s-%u%s", base_dirpath, base_file_name,
					output_index, base_file_extension);
	...
	if (in_video_st->codec->codec != NULL)
		avcodec_close(in_video_st->codec);
	if (num_segments < MAX_SEGMENTS) {
		...
		if (num_segments > 0) {
			...
			num_segments++;
			write_index_file(output_index_file, tmp_output_index_file, segmentLength, num_segments,
					actual_segment_durations, listofs, base_file_name, base_file_extension, 1);
		}
	}
...
int main(int argc, const char *argv[]) {
	...
	if (parseCommandLine(argc, argv, inputFilename, playlistFilename, baseDirName, baseFileName, baseFileExtension,
			&segmentLength, &listlen, &quiet, &version, &usage, &persist) != 0)
		return 0;
	...
	ret = segment(inputFilename, baseDirName, playlistFilename, baseFileName, baseFileExtension, segmentLength,
			listlen);
	return ret;
}
